...
BUCKET_ACCOUNTING="${NEFT_S3_BUCKET_ACCOUNTING_EXPORTS:-accounting-exports}"
BUCKET_DOCUMENTS="${NEFT_S3_BUCKET_DOCUMENTS:-neft-documents}"
...
if [ "$BUCKET_ACCOUNTING" != "$BUCKET_MAIN" ] && [ "$BUCKET_ACCOUNTING" != "$BUCKET_INVOICES" ] && [ "$BUCKET_ACCOUNTING" != "$BUCKET_PAYOUTS" ]; then
  ensure_bucket "$BUCKET_ACCOUNTING"
fi
if [ "$BUCKET_DOCUMENTS" != "$BUCKET_MAIN" ] && [ "$BUCKET_DOCUMENTS" != "$BUCKET_INVOICES" ] && [ "$BUCKET_DOCUMENTS" != "$BUCKET_PAYOUTS" ] && [ "$BUCKET_DOCUMENTS" != "$BUCKET_ACCOUNTING" ]; then
  ensure_bucket "$BUCKET_DOCUMENTS"
fi
