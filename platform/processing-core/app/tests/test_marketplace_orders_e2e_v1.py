...
from app.models.marketplace_settlement import MarketplaceAdjustment, MarketplaceSettlementItem, MarketplaceSettlementSnapshot
...
def _build_app(session_factory: sessionmaker) -> FastAPI:
    ...
    app.include_router(admin_orders_router, prefix="/api/core/v1/admin")
...
def _build_session_factory() -> tuple[sessionmaker, object]:
    ...
    for table in MARKETPLACE_ORDER_TEST_TABLES:
...
def test_marketplace_orders_lifecycle_e2e() -> None:
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            settlement_response = client.get(f"/api/core/v1/admin/marketplace/orders/{order_id}/settlement-snapshot")
            assert settlement_response.status_code == 200
            settlement_payload = settlement_response.json()
            assert settlement_payload["order_id"] == order_id
            assert settlement_payload["hash"]

            detail_response = client.get(f"/api/core/v1/admin/marketplace/orders/{order_id}")
            assert detail_response.status_code == 200
            detail_payload = detail_response.json()
            assert detail_payload["payment_status"] == "PAID"
            assert detail_payload["payment_method"] == "NEFT_INTERNAL"

            events_response = client.get(f"/api/core/v1/admin/marketplace/orders/{order_id}/events")
        ...
        for table in reversed(MARKETPLACE_ORDER_TEST_TABLES):
            table.drop(bind=engine)
        engine.dispose()


def test_marketplace_orders_proof_required() -> None:
    SessionLocal, engine = _build_session_factory()
    try:
        app = _build_app(SessionLocal)
        with TestClient(app) as client:
