...
from app.tests._crm_test_harness import CRM_ONBOARDING_TEST_TABLES, crm_session_context


def test_onboarding_recompute_progression():
    with crm_session_context(tables=CRM_ONBOARDING_TEST_TABLES) as db:
        client = CRMClient(
            id="client-1",
            tenant_id=1,
            legal_name="Client",
...
def test_onboarding_action_meta_accumulates_across_steps():
    with crm_session_context(tables=CRM_ONBOARDING_TEST_TABLES) as db:
        client = CRMClient(
            id="client-3",
            tenant_id=1,
        ...
        onboarding.advance(
            db,
            client_id=client.id,
            action=onboarding.OnboardingAction.REQUEST_LEGAL,
            request_ctx=None,
        )
        onboarding.advance(
            db,
        ...
        current = repository.get_onboarding_state(db, client_id=client.id)
        assert current is not None
        assert current.meta["legal_requested"] is True
        assert current.meta["legal_accepted"] is True
        assert current.meta["contract_signed"] is True
