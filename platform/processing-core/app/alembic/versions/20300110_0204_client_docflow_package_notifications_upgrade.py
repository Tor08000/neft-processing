...
from alembic_helpers import DB_SCHEMA, column_exists, constraint_exists, index_exists
...
def get_column_data_type(bind, schema: str, table_name: str, column_name: str) -> str | None:
    result = bind.execute(
        sa.text(
            """
            SELECT data_type
...
def upgrade() -> None:
    ...
    if not column_exists(bind, "client_document_packages", "sha256", schema=DB_SCHEMA):
        op.add_column("client_document_packages", sa.Column("sha256", sa.Text(), nullable=True), schema=DB_SCHEMA)
    if not column_exists(bind, "client_document_packages", "expires_at", schema=DB_SCHEMA):
        op.add_column("client_document_packages", sa.Column("expires_at", sa.DateTime(timezone=True), nullable=True), schema=DB_SCHEMA)
    if not column_exists(bind, "client_document_packages", "error_code", schema=DB_SCHEMA):
        op.add_column("client_document_packages", sa.Column("error_code", sa.Text(), nullable=True), schema=DB_SCHEMA)

    if not constraint_exists(bind, "client_document_package_items", "uq_client_document_package_items_package_doc", schema=DB_SCHEMA):
    ...
    if not column_exists(bind, "client_docflow_notifications", "kind", schema=DB_SCHEMA):
        op.add_column("client_docflow_notifications", sa.Column("kind", sa.Text(), nullable=True), schema=DB_SCHEMA)
    if not column_exists(bind, "client_docflow_notifications", "message", schema=DB_SCHEMA):
        op.add_column("client_docflow_notifications", sa.Column("message", sa.Text(), nullable=True), schema=DB_SCHEMA)
    if not column_exists(bind, "client_docflow_notifications", "payload", schema=DB_SCHEMA):
        op.add_column("client_docflow_notifications", sa.Column("payload", sa.JSON(), nullable=True), schema=DB_SCHEMA)
    if not column_exists(bind, "client_docflow_notifications", "severity", schema=DB_SCHEMA):
    ...
    if not column_exists(bind, "client_docflow_notifications", "is_read", schema=DB_SCHEMA):
    ...
    if not column_exists(bind, "client_docflow_notifications", "dedupe_key", schema=DB_SCHEMA):
    ...
    payload_exists = column_exists(bind, "client_docflow_notifications", "payload", schema=DB_SCHEMA)
    meta_json_exists = column_exists(bind, "client_docflow_notifications", "meta_json", schema=DB_SCHEMA)
    payload_data_type = get_column_data_type(bind, DB_SCHEMA, "client_docflow_notifications", "payload") if payload_exists else None
    ...
    if not index_exists(bind, "ix_client_docflow_notifications_user_created", schema=DB_SCHEMA):
        op.create_index(
    ...
    if not index_exists(bind, "ix_client_docflow_notifications_dedupe_key", schema=DB_SCHEMA):
        op.create_index(
...
def downgrade() -> None:
    ...
    if index_exists(bind, "ix_client_docflow_notifications_dedupe_key", schema=DB_SCHEMA):
        op.drop_index("ix_client_docflow_notifications_dedupe_key", table_name="client_docflow_notifications", schema=DB_SCHEMA)
    if index_exists(bind, "ix_client_docflow_notifications_user_created", schema=DB_SCHEMA):
        op.drop_index("ix_client_docflow_notifications_user_created", table_name="client_docflow_notifications", schema=DB_SCHEMA)
    if column_exists(bind, "client_docflow_notifications", "dedupe_key", schema=DB_SCHEMA):
        op.drop_column("client_docflow_notifications", "dedupe_key", schema=DB_SCHEMA)
    if column_exists(bind, "client_docflow_notifications", "is_read", schema=DB_SCHEMA):
        op.drop_column("client_docflow_notifications", "is_read", schema=DB_SCHEMA)
    if column_exists(bind, "client_docflow_notifications", "severity", schema=DB_SCHEMA):
        op.drop_column("client_docflow_notifications", "severity", schema=DB_SCHEMA)
    if column_exists(bind, "client_docflow_notifications", "payload", schema=DB_SCHEMA):
        op.drop_column("client_docflow_notifications", "payload", schema=DB_SCHEMA)
    if column_exists(bind, "client_docflow_notifications", "message", schema=DB_SCHEMA):
        op.drop_column("client_docflow_notifications", "message", schema=DB_SCHEMA)
    if column_exists(bind, "client_docflow_notifications", "kind", schema=DB_SCHEMA):
        op.drop_column("client_docflow_notifications", "kind", schema=DB_SCHEMA)

    if constraint_exists(bind, "client_document_package_items", "uq_client_document_package_items_package_doc", schema=DB_SCHEMA):
        op.drop_constraint("uq_client_document_package_items_package_doc", "client_document_package_items", schema=DB_SCHEMA)
    if column_exists(bind, "client_document_packages", "error_code", schema=DB_SCHEMA):
        op.drop_column("client_document_packages", "error_code", schema=DB_SCHEMA)
    if column_exists(bind, "client_document_packages", "expires_at", schema=DB_SCHEMA):
        op.drop_column("client_document_packages", "expires_at", schema=DB_SCHEMA)
    if column_exists(bind, "client_document_packages", "sha256", schema=DB_SCHEMA):
