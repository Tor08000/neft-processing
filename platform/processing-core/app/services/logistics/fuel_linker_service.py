...
def _nearest_distance_to_trip(db: Session, trip_id: str, lat: float, lon: float) -> float | None:
    stops = db.query(LogisticsStop).join(LogisticsRoute, LogisticsRoute.id == LogisticsStop.route_id).filter(LogisticsRoute.order_id == trip_id).all()
    tracking = db.query(LogisticsTrackingEvent).filter(LogisticsTrackingEvent.order_id == trip_id).order_by(LogisticsTrackingEvent.ts.desc()).limit(200).all()
...
def run_linker(db: Session, *, date_from: datetime, date_to: datetime) -> FuelLinkerStats:
    txs = (
        db.query(FuelTransaction)
        .filter(FuelTransaction.occurred_at >= date_from)
        .filter(FuelTransaction.occurred_at <= date_to)
        .order_by(FuelTransaction.occurred_at.asc())
        .all()
    )
    linked = 0
    unlinked = 0
...
def _trip_distance_km(db: Session, trip_id: str) -> float:
    route = db.query(LogisticsRoute).filter(LogisticsRoute.order_id == trip_id).order_by(LogisticsRoute.version.desc()).first()
...
def list_unlinked(db: Session, *, date_from: datetime, date_to: datetime, limit: int, offset: int) -> list[dict]:
    txs = (
        db.query(FuelTransaction)
        .filter(FuelTransaction.occurred_at >= date_from)
        .filter(FuelTransaction.occurred_at <= date_to)
...
def link_manually(db: Session, *, trip_id: str, fuel_tx_id: str) -> LogisticsFuelLink:
    ...
    if existing:
        return existing
    tx = db.query(FuelTransaction).filter(FuelTransaction.id == fuel_tx_id).one()
...
def fuel_report(db: Session, *, date_from: datetime, date_to: datetime, group_by: str) -> list[dict]:
