...
def render_cards_csv(db: Session, *, client_id: str, filters: dict[str, Any]) -> ExportRenderResult:
    ...
    cards = query.order_by(FuelCard.created_at.desc()).limit(MAX_EXPORT_ROWS + 1).all()
    if len(cards) > MAX_EXPORT_ROWS:
        raise ExportRenderLimitError("too_large")
...
def render_users_csv(db: Session, *, client_id: str, filters: dict[str, Any]) -> ExportRenderResult:
    ...
    users = query.order_by(ClientEmployee.created_at.desc()).limit(MAX_EXPORT_ROWS + 1).all()
...
def render_transactions_csv(db: Session, *, client_id: str, filters: dict[str, Any]) -> ExportRenderResult:
    ...
    operations = query.order_by(Operation.created_at.desc()).limit(MAX_EXPORT_ROWS + 1).all()
    if len(operations) > MAX_EXPORT_ROWS:
        raise ExportRenderLimitError("too_large")
...
def render_documents_csv(db: Session, *, client_id: str, filters: dict[str, Any]) -> ExportRenderResult:
    ...
    documents = query.order_by(Document.period_to.desc()).limit(MAX_EXPORT_ROWS + 1).all()
...
def render_audit_csv(
    ...
    *,
    ...
    logs = query.order_by(AuditLog.ts.desc(), AuditLog.id.desc()).limit(MAX_EXPORT_ROWS + 1).all()
    if len(logs) > MAX_EXPORT_ROWS:
        raise ExportRenderLimitError("too_large")
...
def render_support_csv(
    ...
    *,
    ...
    tickets = query.order_by(SupportTicket.created_at.desc()).limit(MAX_EXPORT_ROWS + 1).all()
    if len(tickets) > MAX_EXPORT_ROWS:
        raise ExportRenderLimitError("too_large")
