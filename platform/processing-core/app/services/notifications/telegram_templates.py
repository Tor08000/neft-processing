...
def _render_links(payload: dict[str, Any]) -> list[str]:
    links = ["вЂў Open in portal: /fleet/notifications"]
    link_type = payload.get("link_type")
    link_id = payload.get("link_id")
    if link_type == "card" and link_id:
        links.append(f"вЂў Card details: /fleet/cards/{link_id}")
    return links
...
def render_telegram_message(payload: dict[str, Any]) -> str:
    ...
    if event_type == "LIMIT_BREACH":
        ...
        lines = [
            f"в›ЅпёЏ NEFT Fleet Alert вЂ” LIMIT_BREACH ({severity})",
            _render_card_line(payload),
            _render_group_line(payload),
            f"Observed: {_format_value(observed)} в‚Ѕ {period or ''}".strip(),
            f"Limit: {_format_value(threshold)} в‚Ѕ",
            f"Delta: {_format_value(delta)}",
            f"Why: {breach_type} breach" if breach_type else "Why: вЂ”",
    ...
    if event_type == "ANOMALY":
        ...
        occurred_at = summary.get("occurred_at")
        lines = [
            f"рџ“€ NEFT Fleet Alert вЂ” ANOMALY ({severity})",
    ...
    if event_type == "POLICY_ACTION":
        ...
        lines = [
            "рџ›Ў Policy action applied" + status_suffix,
            f"Action: {_format_value(action)}",
            f"Reason: {_format_value(reason)}" if reason else "Reason: вЂ”",
            _render_card_line(payload),
            _render_group_line(payload),
            "Open: /fleet/notifications",
        ]
    ...
    if event_type == "INGEST_FAILED":
        lines = [
            f"вљ пёЏ NEFT Fleet Alert вЂ” INGEST_FAILED ({severity})",
    ...
    title = f"NEFT Fleet Alert вЂ” {event_type or 'EVENT'} ({severity})"
