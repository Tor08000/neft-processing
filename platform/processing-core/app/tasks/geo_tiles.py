...
from app.services.geo_analytics import build_geo_tiles_for_day, geo_tiles_backfill
...
def geo_tiles_backfill_task(days: int = 7, zooms: list[int] | None = None) -> dict[str, object]:
    target_zooms = zooms or ZOOMS
    with SessionLocal() as db:
    ...
    return {
        "days": days,
        "zooms": target_zooms,
        "rebuilt": [{"day": day.isoformat(), "zoom": zoom, "tiles": count} for day, zoom, count in rebuilt],
    }


__all__ = ["geo_tiles_refresh_task", "geo_tiles_backfill_task", "ZOOMS"]
