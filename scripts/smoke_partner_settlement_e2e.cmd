...
set "SCRIPT_NAME=smoke_partner_settlement_e2e"
...
set "STEP=7_balance"
call :log "[%STEP%] GET %CORE_PARTNER_URL%/balance"
set "HTTP_HEADER=%PARTNER_AUTH_HEADER%"
call :http_get "%CORE_PARTNER_URL%/balance" "%TEMP%\partner_balance.json"
call :log "Status: !LAST_STATUS!"
call :append_response "%TEMP%\partner_balance.json"
if not "!LAST_STATUS!"=="200" call :fail "%STEP%"
call :expect_contains "%TEMP%\partner_balance.json" "balance_available" || call :fail "%STEP%"

set "STEP=8_request_payout"
set "PAYOUT_PAYLOAD={""amount"":1000,""currency"":""RUB""}"
call :log "[%STEP%] POST %CORE_PARTNER_URL%/payouts/request"
set "HTTP_HEADER=%PARTNER_AUTH_HEADER%"
call :http_post "%CORE_PARTNER_URL%/payouts/request" "!PAYOUT_PAYLOAD!" "%TEMP%\payout_request.json"
call :log "Status: !LAST_STATUS!"
call :append_response "%TEMP%\payout_request.json"
if not "!LAST_STATUS!"=="201" call :fail "%STEP%"
call :extract_json_field "%TEMP%\payout_request.json" "id" PAYOUT_ID
if "!PAYOUT_ID!"=="" call :fail "%STEP%"

set "STEP=9_admin_approve"
call :log "[%STEP%] POST %CORE_ADMIN_URL%/partners/payouts/!PAYOUT_ID!/approve"
set "HTTP_HEADER=%ADMIN_AUTH_HEADER%"
call :http_post "%CORE_ADMIN_URL%/partners/payouts/!PAYOUT_ID!/approve" "{}" "%TEMP%\payout_approve.json"
call :log "Status: !LAST_STATUS!"
call :append_response "%TEMP%\payout_approve.json"
if not "!LAST_STATUS!"=="200" call :fail "%STEP%"

set "STEP=10_admin_mark_paid"
call :log "[%STEP%] POST %CORE_ADMIN_URL%/partners/payouts/!PAYOUT_ID!/mark-paid"
set "HTTP_HEADER=%ADMIN_AUTH_HEADER%"
call :http_post "%CORE_ADMIN_URL%/partners/payouts/!PAYOUT_ID!/mark-paid" "{}" "%TEMP%\payout_paid.json"
call :log "Status: !LAST_STATUS!"
call :append_response "%TEMP%\payout_paid.json"
if not "!LAST_STATUS!"=="200" call :fail "%STEP%"

call :log "E2E_PARTNER_SETTLEMENT: PASS"
echo E2E_PARTNER_SETTLEMENT: PASS
exit /b 0

:fail
set "FAILED_STEP=%~1"
call :log "E2E_PARTNER_SETTLEMENT: FAIL at %FAILED_STEP%"
echo E2E_PARTNER_SETTLEMENT: FAIL at %FAILED_STEP%
exit /b 1
