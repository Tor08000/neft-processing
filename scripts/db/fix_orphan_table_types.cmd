...
rem Add additional table names here when orphan composite types surface.
for %%T in (client_onboarding_state) do (
  set TYPE_NAME=%%T
  echo [fix-orphan-table-types] checking %SCHEMA%.%%T on %TARGET% ...
...
with engine.begin() as conn:
    ...
    if type_exists and not table_exists:
        ...
        print(f'dropped orphan table type {schema}.{type_name}')
    else:
        print(f'no orphan table type for {schema}.{type_name}')
