...
curl -s -S -H "%AUTH_HEADER%" "%CORE_BI%/partner/performance?from=2024-01-01&to=2024-01-31&partner_id=%PARTNER_ID%" > partner_perf.json
python -c "import json; d=json.load(open('partner_perf.json')); items=d.get('items') or []; assert isinstance(items, list); print('OK items', len(items))" || exit /b 1
