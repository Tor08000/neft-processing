...
set "TMP_DIR=%~dp0_tmp\smoke_bi_ops_dashboard"
...
call :get_step "[4/4] Ops SLA dashboard" "%CORE_BI%/ops/sla?from=2024-01-01&to=2024-01-31" "%AUTH_HEADER%" "200" "%TMP_DIR%\ops_sla.json" || exit /b 1
python -c "import json; d=json.load(open(r'%TMP_DIR%\\ops_sla.json')); totals=d.get('totals') or {}; required=['total_orders','sla_breaches']; missing=[k for k in required if k not in totals]; assert not missing, f'Missing totals: {missing}'; assert all(totals[k] is not None for k in required); print('OK totals', totals)" || exit /b 1
