...
curl -s -S -H "%AUTH_HEADER%" "%CORE_BI%/client/spend?from=2024-01-01&to=2024-01-31&client_id=%CLIENT_ID%" > client_spend.json
python -c "import json; d=json.load(open('client_spend.json')); items=d.get('items') or []; assert isinstance(items, list); print('OK items', len(items))" || exit /b 1
