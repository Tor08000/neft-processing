...
import { fetchClientAnalyticsDriverDrill } from "../api/clientAnalytics";
...
export function AnalyticsDriverPage() {
  const { user } = useAuth();
  ...
  const driverLabel = items[0]?.driver_label ?? userId ?? "Водитель";
  ...
  const loadData = useCallback(
    async (cursor: string | null, append: boolean) => {
      if (!user?.clientId || !userId || !dateFrom || !dateTo) return;
      try {
        if (append) {
        ...
        setError(null);
        const response = await fetchClientAnalyticsDriverDrill(user, userId, {
          from: dateFrom,
          to: dateTo,
          limit: 50,
          cursor,
  ...
  if (!userId || !dateFrom || !dateTo) {
    return <StatusPage title="Некорректный период" description="Укажите водителя и период для детализации." />;
  }
  ...
  return (
    <DrilldownLayout
      title={`Транзакции по водителю ${driverLabel}`}
      subtitle="Детализация операций по выбранному водителю."
