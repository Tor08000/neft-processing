...
export const markInvoicePaid = async (invoiceId: string, payload: { reason: string; correlation_id: string }) =>
  apiPost(`/api/core/v1/admin/finance/invoices/${invoiceId}/mark-paid`, payload);

export const voidInvoice = async (invoiceId: string, payload: { reason: string; correlation_id: string }) =>
  apiPost(`/api/core/v1/admin/finance/invoices/${invoiceId}/void`, payload);

export const markInvoiceOverdue = async (invoiceId: string, payload: { reason: string; correlation_id: string }) =>
  apiPost(`/api/core/v1/admin/finance/invoices/${invoiceId}/mark-overdue`, payload);
...
export const approvePaymentIntake = async (intakeId: number, payload: { reason: string; correlation_id: string }) =>
  apiPost(`/api/core/v1/admin/finance/payment-intakes/${intakeId}/approve`, payload);

export const rejectPaymentIntake = async (intakeId: number, payload: { reason: string; correlation_id: string }) =>
  apiPost(`/api/core/v1/admin/finance/payment-intakes/${intakeId}/reject`, payload);
...
) => request(`/v1/admin/finance/payouts/${payoutId}/approve`, { method: "POST", body: JSON.stringify(payload) }, token);

export const rejectPayout = async (
  token: string,
  payoutId: string,
  payload: { reason: string; correlation_id: string },
) => request(`/v1/admin/finance/payouts/${payoutId}/reject`, { method: "POST", body: JSON.stringify(payload) }, token);

export const markPayoutPaid = async (
  token: string,
  payoutId: string,
  payload: { reason: string; correlation_id: string },
) => request(`/v1/admin/finance/payouts/${payoutId}/mark-paid`, { method: "POST", body: JSON.stringify(payload) }, token);

export const fetchPartnerLedger = async (partnerId: string): Promise<PartnerLedgerSnapshot> =>
  apiGet(`/api/core/v1/admin/finance/partners/${partnerId}/ledger`);

export const fetchPartnerSettlement = async (partnerId: string): Promise<PartnerSettlementSnapshot> =>
  apiGet(`/api/core/v1/admin/finance/partners/${partnerId}/settlement`);
