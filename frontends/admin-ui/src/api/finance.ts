...
export const fetchFinanceOverview = async (window: "24h" | "7d" = "24h"): Promise<FinanceOverview> =>
  apiGet("/finance/overview", { window });
...
export const fetchPayoutQueue = async (params: {
  ...
  limit?: number;
  offset?: number;
}): Promise<PayoutQueueListResponse> => apiGet("/finance/payouts", params);

export const fetchPayoutDetail = async (payoutId: string): Promise<PayoutDetail> =>
  apiGet(`/finance/payouts/${payoutId}`);

export const approvePayout = async (
  token: string,
  payoutId: string,
  payload: { reason: string; correlation_id: string },
) => request(`/finance/payouts/${payoutId}/approve`, { method: "POST", body: JSON.stringify(payload) }, token);

export const rejectPayout = async (
  token: string,
  payoutId: string,
  payload: { reason: string; correlation_id: string },
) => request(`/finance/payouts/${payoutId}/reject`, { method: "POST", body: JSON.stringify(payload) }, token);

export const markPayoutPaid = async (
  token: string,
  payoutId: string,
  payload: { reason: string; correlation_id: string },
) => request(`/finance/payouts/${payoutId}/mark-paid`, { method: "POST", body: JSON.stringify(payload) }, token);

export const fetchPartnerLedger = async (partnerId: string): Promise<PartnerLedgerSnapshot> =>
  apiGet(`/finance/partners/${partnerId}/ledger`);

export const fetchPartnerSettlement = async (partnerId: string): Promise<PartnerSettlementSnapshot> =>
  apiGet(`/finance/partners/${partnerId}/settlement`);
