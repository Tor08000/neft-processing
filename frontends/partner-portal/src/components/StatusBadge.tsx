...
const toneMap = new Map<string, StatusBadgeProps["tone"]>([
  ["active", "success"],
  ["online", "success"],
  ["settled", "success"],
  ["delivered", "success"],
...
export function StatusBadge({ status, tone }: StatusBadgeProps) {
  ...
  const key = status ? statusLabelMap.get(String(status).toUpperCase()) : null;
  const label = key ? t(key) : normalized;
  const resolvedTone =
    tone ?? toneMap.get(String(status).toLowerCase()) ?? "neutral";
