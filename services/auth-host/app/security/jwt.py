...
from neft_shared.settings import Settings
...
def issue_tokens(sub: str) -> dict:
    ...
    refresh = jwt.encode({"sub": sub, "exp": ref_exp, "typ": "refresh", "jti": jti}, settings.jwt_secret, algorithm=ALG)
    return {"access_token": access, "refresh_token": refresh, "token_type": "bearer", "jti": jti, "refresh_exp": int(ref_exp.timestamp())}
