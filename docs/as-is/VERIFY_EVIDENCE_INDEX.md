...
| Invoice state machine smoke | VERIFIED_BY_SMOKE_SCRIPT (conditional) | `scripts/smoke_invoice_state_machine.cmd` | invoice state transitions via smoke script (SKIP when no invoices) |
