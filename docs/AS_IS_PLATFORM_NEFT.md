...
NEFT Platform в текущем виде — набор сервисов вокруг процессинга, биллинга и объяснимости, собранный в `docker-compose.yml`:
* **core-api (FastAPI)** — процессинг, финансы, логистика, CRM, explainability и admin API (`platform/processing-core/app`).
* **auth-host (FastAPI)** — авторизация админ/клиентских пользователей, минимальная БД на SQL (`auth-host.md`).
* **ai-service (FastAPI)** — risk scoring endpoint (stub), используется как внешний scorer для risk v5 (`platform/processing-core/app/services/risk_v5`).
* **Celery workers/beat + Flower** — фоновые задачи (billing, fleet intelligence, replay).
* **infra**: Postgres, Redis, MinIO, Nginx gateway, Prometheus/Grafana, OTel collector + Jaeger.

**Ключевые контуры управления/объяснимости (AS-IS):**
* Unified Explain v1.1: primary/secondary reason, action hints, SLA и escalation (`platform/processing-core/app/services/explain`).
...
* Money Flow v3: граф links/snapshots/replay/CFO explain (`platform/processing-core/app/services/money_flow`).
* Navigator core (адаптеры + snapshots) (`platform/processing-core/app/services/logistics/navigator`).
...
* Fuel домен (authorize/settle/reverse), limits v2 и antifraud v3 (`platform/processing-core/app/services/fuel`).
* Logistics v1/v2 + Navigator core (ETA/deviation snapshots) (`platform/processing-core/app/services/logistics`).
* CRM control plane + subscriptions v2 с proration/tariffs v2 и money flow links (`platform/processing-core/app/services/crm`).
* Unified Explain v1.1 (actions/SLA/escalation) и Ops workflow (escalations/KPI).
* Fleet Intelligence/Control v1–v3, Fleet Assistant, Decision Memory и What-If.
* Money Flow v3 (links/snapshots/replay/CFO explain) + internal ledger invariants.
...
* Критичные домены подтверждены: core-api ✅, documents ✅, risk v3/v4/v5 ✅, accounting_export_batch ✅, money flow v3 ✅, ops workflow ✅, fleet intelligence/control ✅.
...
* Unified Explain v1.1: primary reason governance + actions/SLA/escalation.
* Ops escalations workflow: taxonomy + KPI + SLA report.
...
* Navigator core snapshots/explains.
...
* Fuel: authorize/settle/reverse + antifraud signals + ledger postings.
* Money Flow: links/snapshots/health/replay/CFO explain.
* CRM subscriptions v2: сегменты/правила/прорейты, генерация счетов и money flow links.
* Fleet Intelligence v1–v3: scores → trends → control loop.
* Fleet Assistant/Decision Memory/What-If: рекомендации, память решений, симуляции (без исполнения).
...
* fuel authorize → unified explain → ops escalation (SLA + reason codes).
* fuel settle → ledger → money_flow_links → replay diagnostics.
...
* Navigator по умолчанию использует noop-адаптер (без внешнего провайдера).
* Ops/Fleet контуры не выполняют автодействий без ручного approve/apply.
* What-If simulator не вызывает risk v4/v5 и не исполняет actions (read-only).
...
* Decision Memory v1 — `platform/processing-core/app/services/decision_memory`.
* What-If Simulator v1 — `platform/processing-core/app/services/what_if`.
* Money Flow v3 (links/snapshots/replay/CFO explain) — `platform/processing-core/app/services/money_flow`, `docs/finance/money_flow_v3.md`.
* Navigator core (snapshots/explains) — `platform/processing-core/app/services/logistics/navigator`, `docs/logistics/navigator_core.md`.

**Что не делаем сейчас:**
* Внешний e-sign/EDI/ERP провайдеры (архитектура есть, интеграции нет).
...
* Реальный провайдер маршрутов (только noop navigator).

**Что флагом:**
* `LOGISTICS_NAVIGATOR_ENABLED` / `LOGISTICS_NAVIGATOR_PROVIDER` (`docs/logistics/navigator_core.md`).
* `FLEET_BENCHMARK_USE_TENANT` (benchmarks v1.2, `docs/fleet_intelligence/fleet_assistant_benchmarks_v1_2.md`).
* CRM control plane header `X-CRM-Version` (freeze guardrails, `platform/processing-core/app/routers/admin/crm.py`).
...
| Logistics + Navigator | 65% | Логистика v1/v2 и navigator snapshots есть; провайдеры маршрутов — noop. |
| CRM + subscriptions | 70% | CRM control plane и subscriptions v2 есть; интеграции внешних CRM отсутствуют. |
| Ops workflow | 70% | Escalations + taxonomy + KPI/SLA отчёты есть; операционная дисциплина зависит от процессов. |
| Explain layer | 75% | Unified Explain v1.1 с action/SLA/escalation; coverage зависит от источников данных. |
...
| External integrations (e-sign/EDI/ERP) | 30% | Архитектура в коде частично описана, реализация отсутствует. |
| UI maturity (admin-ui / client portal) | 60% | SPA собираются, функционал зависит от backend. |
...
| Logistics + Navigator | 🟡 | Логистика v1/v2 + navigator snapshots есть, провайдеры маршрутов stub. |
...
| Admin UI / Client portal | 🟡 | SPA собираются, функционал зависит от backend. |
| Observability | 🟡 | Метрики есть, auth-host metrics отсутствуют, traces частично. |
...
* celery beat запускается без явного импорта расписаний (`platform/billing-clearing/app/beat.py`).
* metrics для auth-host отсутствуют (endpoint только в Prometheus config).
* дублирование API префиксов: `/api`, `/api/core`, `/api/v1` сосуществуют (`platform/processing-core/app/main.py`).
* внешние integrations (payouts/settlements, e-sign/EDI/ERP) отсутствуют.
* navigator провайдеры кроме noop не подключены (`docs/logistics/navigator_core.md`).
* ai-service используется как stub scorer; реальный scorer/ML не подключён.
* `money_flow_links` и `money_invariant_snapshots` описаны в моделях без отдельных миграций.
* документооборот ограничен внутренним lifecycle без внешнего подписания.
