...
* Cross-period settlement allocations для платежей/credit notes/возвратов.
* Accounting export batches с детерминированным сериализатором и S3 storage.
* RBAC/policy engine для финансовых действий (finalize/lock/payment/credit/export).
* Legal finalization для documents/closing packages (ISSUED → ACKNOWLEDGED → FINALIZED).
...
* billing period finalize/lock → invoices → payments/credits → settlement allocations.
* accounting export (charges/settlement) → S3 upload → download → confirm.
* documents issue/ack/finalize → immutable chain (hash) → audit.
...
* **Статус:** ✅ — есть идемпотентность, state machine, settlement allocations и аудит.

### Accounting export

* **Назначение:** выгрузки charges/settlement по периодам.
* **Сущности:** `accounting_export_batches`.
...
* **Фоновые процессы:** отсутствуют.
* **Статус:** ✅ — legal finalization lifecycle, hash/document chain, immutable guards, policy checks для ack/finalize.
...
* Таблица `invoice_settlement_allocations` хранит привязку payments/credit notes/refunds к `settlement_period_id`.
...
* Таблица `accounting_export_batches` фиксирует batch: type/format/state, idempotency_key, checksum_sha256.
...
* Идемпотентность: повторный create/export возвращает существующий batch.
...
* `GET /settlement-summary` — settlement allocations summary.
...
* `test_settlement_allocations.py`, `test_accounting_exports.py` — settlement allocations + accounting exports.
...
| Finance (payments/credit notes/refunds) | ✅ | Idempotency, status transitions, settlement allocations и аудит реализованы. |
| Settlement allocations | ✅ | `invoice_settlement_allocations`, cross-period логика и summary отчёты есть. |
| Accounting export | ✅ | Batch lifecycle, deterministic serializer, S3 upload/download/confirm. |
| RBAC/policies (finance) | ✅ | Policy engine защищает finalize/lock/payment/credit/export/payout. |
| Documents (legal finalization) | ✅ | ISSUED→ACKNOWLEDGED→FINALIZED, hash chain, immutability guards. |
