...
* Клиринг: daily batch (связка операций → clearing_batch) + reconcile/mark.
...
* AI scoring — stub (простая логика в `local_llm`).
...
| gateway | gateway/ | infra/api gateway | 80 | core-api, auth-host, ai-service, admin-web, client-web | nginx | ✅ | Роутинг /api/core, /api/auth, /api/ai, /admin, /client. | gateway/nginx.conf |
...
| core-api | platform/processing-core | api | 8001->8000 | postgres, redis, minio | uvicorn app.main:app | 🟡 | Основная бизнес-логика: operations, billing, clearing, payout, risk, admin, client portal. | app/main.py, app/services, app/routers, app/models |
| ai-service | platform/ai-services/risk-scorer | api | 8003->8000 | redis (по compose) | uvicorn app.main:app | 🟡 | Риск скоринг (stub), прометей метрики. | app/main.py, app/api/v1 |
...
* core-api → ai-service: HTTP POST в `AI_SCORE_URL` (`app/services/risk_adapter.py`)
...
* **Статус:** 🟡 — ядро реализовано, но не все процессы (например, notifications) присутствуют.
...
* **Статус:** ✅ ядро lifecycle + PDF + финоперации есть.
...
* **Статус:** ✅ — есть идемпотентность, state machine и аудит.
...
* **Сущности:** `clients`, `merchants`, `terminals`, `documents`, `document_files`, `closing_packages`, `client_actions`, `reconciliation_requests`.
...
* **Фоновые процессы:** отсутствуют.
* **Статус:** 🟡 — CRUD и базовые workflows, но отсутствуют внешние интеграции/полный документооборот.
...
* **Статус:** 🟡 — правила и лимиты реализованы, но ML/AI — stub и есть fallback поведение.
...
* Отдельный документооборот (акты/архив) — частично (Document/ClosingPackage есть, но нет полного workflow и внешних интеграций).
...
- `closing_package_status`: DRAFT, GENERATED, SENT, ACKNOWLEDGED, CANCELLED
- `credit_note_status`: POSTED, FAILED, REVERSED
- `document_file_type`: PDF, XLSX
- `document_status`: DRAFT, GENERATED, SENT, ACKNOWLEDGED, CANCELLED
...
* `app/routers/client_documents.py` — docs/closing packages ack/download.
...
* `platform/processing-core/app/tests/*` — billing pipeline, invoices, payments, refunds, state machine, migrations, limits, risk rules, payouts, settlements, metrics, api endpoints.
...
| Billing | 🟡 | Ядро реализовано, но отсутствуют внешние интеграции и уведомления. |
| Invoices + PDF | ✅ | State machine, PDF generation и storage реализованы. |
| Finance (payments/credit notes/refunds) | ✅ | Idempotency, status transitions и аудит реализованы. |
...
| Risk/AI | 🟡 | Есть rules + stub AI scoring. |
...
7. **Документооборот**: расширить `documents`/`closing_packages` и добавить workflows/архив.
...
18. **Client actions**: завершить API для reconciliation/closing packages.
