...
* **integration-hub (FastAPI + Celery)** ? EDO + webhook self-service v1.1 (retry/replay/SLA/alerts/metrics; partner canonical family `/api/int/v1/webhooks/*`) (`platform/integration-hub`).
* **logistics-service (FastAPI)** — ETA/deviation/explain, explicit transport-vs-compute split: transport `mock`/`integration_hub`, compute `mock`/`osrm` (`platform/logistics-service`).
...
* **Partner Portal MAX**: prices + pricing analytics, integrations (webhooks self-service), services/catalog/offers, orders/refunds, partner finance (balance + ledger), payout requests, invoices/acts, i18n, UX polish (`frontends/partner-portal/src/App.tsx`).
* **Partner Legal & Tax Pack**: юридический профиль, налоговые режимы (read-only), payout gating + legal pack PDF/ZIP (`docs/as-is/PARTNER_LEGAL_AS_IS.md`).
...
* **Webhooks v1.1**: integration-hub-owned partner webhook self-service with replay scheduling, pause/resume, SLA/alerts/metrics, partner UI controls via `/api/int/v1/webhooks/*`; `processing-core` helpdesk inbound webhooks are a separate contour (`platform/integration-hub`, `frontends/partner-portal/src/pages/IntegrationsPage.tsx`, `platform/processing-core/app/routers/helpdesk_webhooks.py`).
...
* **Support / Cases unified contour**: canonical `cases` owner with compatibility `support_requests` tail, client helpdesk tickets linked to cases, partner/client/admin UI aligned to the shared lifecycle (`platform/processing-core/app/routers/cases.py`, `platform/processing-core/app/api/v1/endpoints/support_requests.py`, portal pages).
...
* **OSRM / Integration transport truth**: logistics-service keeps explicit transport-vs-compute split; integration-hub is the canonical external EDO/webhook transport owner with explicit mock/degraded modes and structured provider failures (`platform/logistics-service`, `platform/integration-hub/neft_integration_hub/*`).
...
* Diadok prod credentials не заданы в репозитории; OTP/notifications/email по умолчанию не mock, а explicit degraded/disabled until configured.
...
* Logistics Service (ETA/deviation/explain, explicit transport/compute provider split) — `platform/logistics-service`.
* BI v1.1 (read-only API + exports CSV/JSONL + manifest + ClickHouse sync) — `platform/processing-core/app/services/bi`.
* Support / Cases unified contour — canonical `platform/processing-core/app/routers/cases.py` plus compatibility `platform/processing-core/app/api/v1/endpoints/support_requests.py`.
...
* **crm-service** — compatibility/shadow CRM service (`/health`, `/metrics`, `/api/v1/crm/*`); canonical CRM control plane owner lives in `processing-core` admin CRM.
...
* `GET http://localhost:8001/metrics` → Prometheus-метрики, `core_api_up 1`; optional outbox tables (`email_outbox`, `event_outbox`, `export_jobs`) report zero gauges when absent in partial/dev schemas instead of emitting ERROR stacktraces.
...
* **Client Controls:** `/client/users|services|features` are mounted in `core-api`; users/invitations flow is runtime-verified, while services/features remain read surfaces.
...
| logistics-service | ETA/deviation/explain + explicit transport/compute providers | internal `8000`, `/health`, `/metrics` | `LOGISTICS_PROVIDER`, `LOGISTICS_COMPUTE_PROVIDER`, `USE_MOCK_LOGISTICS`, `INTEGRATION_HUB_BASE_URL`, `OSRM_BASE_URL`, `OSRM_TIMEOUT_SECONDS` | integration-hub (опц.), внешн. OSRM (опц.) | ✅ |
| ai-service | Heuristic risk scoring API with explicit degraded/source semantics | `8003→8000`, `/api/ai/health`, `/metrics` | `API_PREFIX_AI`, `SERVICE_NAME` | redis | 🟡 (heuristic scoring, no real ML owner) |
...
| crm-service | Compatibility/shadow CRM service | internal `8000`, `/health`, `/metrics`, `/api/v1/crm/*` | shadow CRM CRUD | canonical owner lives in `processing-core` admin CRM | 🟡 |
...
* **API:** `/api/v1/invoices/{id}/payments`, `/api/v1/invoices/{id}/refunds`, `/api/core/v1/admin/finance/*`.
...
* **API:** `/v1/edo/dispatch`, `/v1/edo/documents/{id}`, `/v1/webhooks/*` (`platform/integration-hub/neft_integration_hub/main.py`); canonical external family for partner self-service is `/api/int/v1/webhooks/*`.
...
* **Провайдеры:** transport `integration_hub` by default with explicit `mock` only by config (`LOGISTICS_PROVIDER`) и compute `osrm` by default with explicit `mock` only by config (`LOGISTICS_COMPUTE_PROVIDER`, `OSRM_BASE_URL`).
* **Статус:** ✅ — explicit provider split; default compute owner is OSRM, while mock remains an explicit dev/test path rather than a silent fallback.
...
* **Назначение:** единый case owner для support, incidents, order issues и dispute-adjacent intake; `support_requests` сохранён только как compatibility tail.
* **API:** canonical `/api/core/cases` + compatibility `/api/v1/support/requests`.
* **Миграции:** `20291720_0089_cases_v1.py`, `20291660_0088_support_requests.py`, `20300190_0211_support_case_unification.py`.
* **Статус:** ✅ — canonical backend owner, compatibility adapter, admin/client/partner UI surfaces, client marketplace order incidents linked into hidden canonical `/cases*`, and partner marketplace order details expose the same canonical incident trail through partner owner routes. Live browser evidence is frozen in `docs/diag/client-partner-support-marketplace-live-smoke.json`; partner marketplace proof requires canonical demo reseed via `scripts/seed_partner_money_e2e.cmd` plus fresh order/case seed via `scripts/smoke_marketplace_order_loop.cmd`. The same marketplace loop now verifies consequence and settlement-readiness tails explicitly: client/admin consequences return mounted `200` item-list payloads, partner settlement stays `409 SETTLEMENT_NOT_FINALIZED` before finalization, and partner settlement returns `200` with finalized snapshot/hash/net after admin finalization.
...
* **Partner Portal MAX:** prices/pricing analytics/integrations/webhooks self-service/services/catalog/offers/orders/refunds/finance/payouts/documents + i18n.
...
* **AI сервис:** `/api/v1/score` (legacy heuristic), `/api/v1/risk-score` v2 + `/admin/ai/train-model`, `/admin/ai/update-model`.
* **Статус:** ✅ — decision engine пишет audit/decision_results; ai-service остаётся heuristic scorer с explicit degraded/source truth, не trained-model owner.
...
* **Money Flow**:
  ...
  * `POST /api/core/v1/admin/money/replay`
  * `GET /api/core/v1/admin/money/cfo-explain` (`app/routers/admin/money_flow.py`)
...
| Support / Cases unified contour | 80% | Canonical `cases` owner реализован; `support_requests` живёт как compatibility tail, portal/admin UX выровнен к одному lifecycle. |
...
**Note on `reports_billing` rows below**

* `app/api/v1/endpoints/reports_billing.py` has router prefix `/api/v1/reports`, so the decorator table below shows declared suffixes like `/billing/daily` and `/turnover`, not the final mounted paths.
* Final mounted routes stay `/api/v1/reports/*` and remain explicit compatibility tails:
  * public/no-auth reads: `/api/v1/reports/billing/daily`, `/api/v1/reports/billing/summary`, `/api/v1/reports/turnover`, `/api/v1/reports/turnover/export`
  * admin-gated compatibility trigger: `POST /api/v1/reports/billing/summary/rebuild`
...
| Integration Hub (EDO/Webhooks) | 🟡 | Webhooks v1.1 + EDO endpoints/metrics есть; mock-by-default убран, live provider readiness зависит от credentials and explicit transport wiring. |
| BI / Exports v1.1 | 🟡 | BI API + exports есть, ClickHouse sync через флаг. |
| Support / Cases unified contour | 🟡 | Canonical `cases` owner реализован; `support_requests` остаётся compatibility tail, portal/admin UX выровнен к одному lifecycle. |
...
* ai-service используется как heuristic scorer; реальный ML не подключён, а unsupported/degraded paths должны быть explicit.
...
| crm-service | 🟡 | Compatibility/shadow CRM routes; not canonical owner. | internal 8000 |
