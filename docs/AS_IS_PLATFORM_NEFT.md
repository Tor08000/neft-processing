...
* **infra** — postgres/redis/minio/clickhouse/gateway/prometheus/grafana/otel/jaeger.
...
* **integration-hub** — отдельный сервис (в compose не поднимается).
* **partner-portal** — отдельный фронтенд (в compose не поднимается по умолчанию).
...
* Статус: SPA в репозитории (Vite), запуск отдельным контейнером не входит в compose по умолчанию.
...
| integration-hub | EDO + webhooks v1.1 (replay/SLA/alerts) | `8000`, `/health`, `/metrics` | `INTEGRATION_HUB_DATABASE_URL`, `WEBHOOK_*`, `DIADOK_*` | redis, DB, minio | ✅ (отдельный сервис) |
...
| partner-portal | Partner portal MAX | отдельно | `VITE_API_BASE` | gateway | ✅ (отдельный фронтенд) |
